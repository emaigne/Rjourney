...
La subtilité ici est sur le `=`, on n’utilise le `:=` que pour créer une
...
On aurait pu créer ces variables directement dans le jeu de données.
Pour ça on remplace la fonction `.()` par la fonction \``  `:=`()` \`:
