...
Mais `data.table` fournit la fonction `setDF()` moins gourmande en ressources (utile pour les gros datasets) puisqu'il n'oblige pas à copier l'objet en mémoire, contrairement au traditionnel `<-`.
...
Puisqu'on parle de l'allocation mémoire, il faut savoir que `data.table` est similaire à de l'orienté objet. 
...
Pour créer un "vrai"  nouvel objet en mémoire, on utilise alors la fonction `copy()` :
...
La fonction *`fread`* vient avec les options habituelles (`sep`, `nrows`, ...) mais très souvent trouve toute seule la bonne combinaison. C'est rare que j'ai besoin de les spécifier... Très pratique toc_depth: 2!
...
On aurait pu aussi ne pas rajouter la variable dans l'object lui même mais créer un vecteur sans redondance avec la fonction de sélection `.()` directement au moment du calcul :
...
Si on avait voulu créer ces variables directement dans le jeu de données, on va remplacer la fonction ` .() `  par la fonction `` `:=`()` `:
